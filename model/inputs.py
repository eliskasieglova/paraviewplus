...
class SurfacePoints(DataPoints):
    ...
    def _apply_plot_layout(self, ax, variable_name):
        ...
        if self.get_title(variable_name) == "unknown":
            ...
            self.ylims = self.get_layout("ylims", variable_name)
            self.yticks = self.get_layout("yticks", variable_name)
        ...
        self.x_axis_label = ax.set_xlabel("Time")
        self.y_axis_label = ax.set_ylabel(units)
    ...
    def _plot_legend(self, ax):
        ...
        legend = ax.legend(loc='center', bbox_to_anchor=(0.5, 1.05), ncol=4, fontsize=8, frameon=False)
        return legend
    ...
    def _plot_note_text(self, variable_name):
        ...
        note_text = plt.figtext(0.5, 0.05,r"$\mathbfit{" + "Note:" + "}$ " + self.note_content,
    ...
    def _build_plot(self, simulation, aois, variable_name, colors, show=False):
        ...
        for idx, aoi in enumerate(aois):
            ...
            line, = plt.plot(timesteps, avg_values, color=colors[idx], label=f"Area {letters[idx]}")
            self.aoi_lines[idx] = line
...
class AirPoints(DataPoints):
    ...
    def plot_windflow(self, time, surfacemesh=gpd.read_file("data/surface_triangle_SHP.shp"),
                        surfacepoints=gpd.read_file("data/surface_point_SHP.shp"),
                        threshold=2,
                        dims=3):
