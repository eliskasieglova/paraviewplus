...
- done -  add the ability to export multliple colors
...
- fix the slices choosing process
- fix the shrinking issue
- done - add the resolution and buffer functionality
...
class TimeSeriesRes(ctk.CTkFrame):
    def __init__(self,parent, surfpoints_inputs,airpoints_inputs, surfmesh_input,surfdata_points, airdata_input, var_input):
...
class SimCompRes(ctk.CTkFrame):
    def __init__(self,parent, surfpoints_inputs,surfdata_points, aoi_list, sims_list, var_input ):
...
class SimResRes(ctk.CTkFrame):
    def __init__(self,parent, surfacepoints_input, surface_data_input , aoi_list, var_list):
        super().__init__(parent, fg_color = "gray95", corner_radius=0)
        self.pack(side='top', fill= 'both', expand = True)
...
class UTCICategoryRes(ctk.CTkFrame):
    def __init__(self, parent, surfpoints_inputs, surfdata_points, surfmesh_input):
        ...
        side_bar_frame2 = ctk.CTkScrollableFrame(side_bar_frame)
        side_bar_frame2.pack(side='top',fill='both',expand=True, pady=6,padx=6)
        ...
        checkbox_0 = ctk.CTkCheckBox(export_frame, text=stress_categories[0][0],border_width = 2,variable=self.check_var_0, onvalue="on", offvalue="off")#, state='disabled')
        checkbox_0.grid(column=0, row=5, columnspan=2, stick='WE', pady=2, padx=2)  
        
        self.check_var_1 = ctk.StringVar(value="off")
        checkbox_1 = ctk.CTkCheckBox(export_frame, text=stress_categories[1][0],border_width = 2,variable=self.check_var_1, onvalue="on", offvalue="off")#, state='disabled')
        checkbox_1.grid(column=0, row=6, columnspan=2, stick='WE', pady=2, padx=2)  
        
        self.check_var_2 = ctk.StringVar(value="off")
        checkbox_2 = ctk.CTkCheckBox(export_frame, text=stress_categories[2][0],border_width = 2,variable=self.check_var_2, onvalue="on", offvalue="off")#, state='disabled')
        checkbox_2.grid(column=0, row=7, columnspan=2, stick='WE', pady=2, padx=2)  
        
        self.check_var_3 = ctk.StringVar(value="off")
        checkbox_3 = ctk.CTkCheckBox(export_frame, text=stress_categories[3][0],border_width = 2,variable=self.check_var_3, onvalue="on", offvalue="off")#, state='disabled')
        checkbox_3.grid(column=0, row=8, columnspan=2, stick='WE', pady=2, padx=2)  

        self.check_var_4 = ctk.StringVar(value="off")
        checkbox_4 = ctk.CTkCheckBox(export_frame, text=stress_categories[4][0],border_width = 2,variable=self.check_var_4, onvalue="on", offvalue="off")#, state='disabled')
        checkbox_4.grid(column=0, row=9, columnspan=2, stick='WE', pady=2, padx=2)     
...
class SliceRes(ctk.CTkFrame):
    def __init__(self, parent, airpoints_input, airdata_input, slices_input):
        ...
        self.slices = slices_input
        self.sl = Slice(self.airpoints, self.airdata, self.slices[0], "Tair")
        ...
        for i in range(len(self.slices)):
            self.create_slice_frame(f"Slice {i+1}")
    ...
    def create_slice_frame(self, placeholder):
        frame = ctk.CTkFrame(self.slices_frame.scrollable_frame)#, fg_color='grey85')
    ...
    def on_radio_select(self):
        selected_area = self.radio_var.get()
        selected_area = selected_area.replace('Slice ', '')
        print(selected_area)
        self.sl.slice = self.slices[int(selected_area)-1]
        self.sl._create_plot(self)
    ...
    def on_resolution_change(self, value):
        print(value)  
        self.sl.set_resolution(value)
        self.sl.update_plot()
        
    def on_buffer_change(self,value):
        self.sl.set_buffer(value)
        self.sl.update_plot()
        print(value)     
...
class Notebook(ttk.Notebook):
    def __init__(self, parent):
        ...
        if self.parent.checked_types[0] == 1:
            self.tab_frame1 = TimeSeriesRes(self, self.parent.surfpoints, self.parent.airpoints, self.parent.surfmesh, self.parent.surfdata, self.parent.airdata, self.parent.sim_series_checked_var)
            self.add(self.tab_frame1, text='Simulation Time Series') 
        
        if self.parent.checked_types[1] == 1:
            self.tab_frame2 = SimResRes(self, self.parent.surfpoints, self.parent.surfdata, self.parent.sim_result_aoi_list, self.parent.sim_result_checked_var)
            self.add(self.tab_frame2, text='Simulation Result')
        
        if self.parent.checked_types[2] == 1:
            self.tab_frame3 = SimCompRes(self, self.parent.surfpoints, self.parent.surfdata, self.parent.sim_comp_aoi_list, self.parent.sims_list, self.parent.sim_comparison_checked_var)
            self.add(self.tab_frame3, text='Simulation Comaprison')
        
        if self.parent.checked_types[3] == 1:
        ...
        if self.parent.checked_types[4] == 1:
            self.tab_frame5 = WindRoseRes(self, self.parent.airpoints, self.parent.airdata)
            self.add(self.tab_frame5, text='Windrose')
        
        if self.parent.checked_types[5] == 1:
            self.tab_frame6 = SliceRes(self, self.parent.airpoints, self.parent.airdata , self.parent.slice_result_slice_list)
...
class ResultWindow(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        # list for all input dictrionaries
        self.checked_types = [0,0,0,0,0,0]
        self.simulation_data = {}
        
        self.sim_series_checked_var = []

        # for the first tab
        self.sim_result_checked_var = []
        self.sim_result_aoi_list = []
        
        # for the comparison tab
        self.sim_comparison_checked_var = []
        self.sim_comp_aoi_list = []
        self.sims_list = []
        
        #for the slices tab
        self.slice_checked_var = []
        self.slice_result_slice_list = []       
    ...
    def initiate_results(self):
        ...
        if self.checked_types[0] == 1:
            self.sim_series_checked_var = self.get_checked_checkboxes('time series')
        if self.checked_types[1] == 1:
            self.sim_result_checked_var = self.get_checked_checkboxes('results')
            self.sim_result_aoi_list = self.get_checked_aoi_and_slices_texts('results')
        if self.checked_types[2] == 1:
            self.sim_comparison_checked_var = self.get_checked_checkboxes('comparison')
            self.sim_comp_aoi_list = self.get_checked_aoi_and_slices_texts('comparison')
            
            #self.sims_list = self.get_checked_data_texts('comparison')
        
        #for the slices tab
        if self.checked_types[5] == 1:
            self.slice_checked_var = self.get_checked_checkboxes('slices')
            self.slice_result_slice_list = self.get_checked_aoi_and_slices_texts('slices')
        ...
        self.sims_list = [self.surfdata, surfdata2]
    ...
    def set_plot_data(self, surfpoint_input, airpoints_input, surfmesh_input, surfdata_input, airdata_input):
        self.surfpoints =  surfpoint_input
        self.airpoints = airpoints_input
        self.surfmesh = surfmesh_input
        self.surfdata = surfdata_input
        self.airdata = airdata_input
        
        

    def add_var_dictionaries(self, input_name, data_dict):
        '''this function adds every dictionary from every sim type requested'''
        self.simulation_data[input_name] = data_dict
        
    def get_checked_aoi_and_slices_texts(self, input_name):
        """
        Extract the AoI entry texts for a specific simulation type where `is_checked` is True.

        Parameters:
            input_name (str): The simulation type (e.g., "time series").

        Returns:
            list: A list of AoI entry texts where `is_checked` is True.
        """
        data = self.simulation_data[input_name]
        data = [ frame["text"]for frame in data.get("aoi_frames", []) if frame["is_checked"] and frame["text"] ]
        
        if input_name == 'slices':
            linestrings = [
                LineString([
                    tuple(map(float, coord.strip("()").split(", ")))
                    for coord in line_str.split("), (") ]) for line_str in data]
            return linestrings
            ...
            polygons = [
                Polygon([
                    tuple(map(float, coord.strip("()").split(", ")))
                    for coord in polygon_str.split("), (") ]) for polygon_str in data]
            return polygons

        
    def get_checked_data_texts(self, input_name):
        """
        Extract the Slice entry texts where `is_checked` is True.

        Returns:
            list: A list of Slice entry texts where `is_checked` is True.
        """
        data = self.simulation_data[input_name]
        return [
            frame["text"]
            for frame in data.get("dataframes", [])
            if frame["is_checked"] and frame["text"]
        ]        
    ...
    def get_checked_checkboxes(self, simulation_names):
        data = self.simulation_data[simulation_names]

        checked_keys = []

        # Iterate through the 'checkboxes' data and filter the checked ones
        for key, state in data.get("checkboxes", {}).items():
            if state["is_checked"]:
                checked_keys.append(key)  # Add the checkbox key to the list if it's checked
